...
func TestControllerConfigCarriesCompaction(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:   "pubsub",
			runner: mustPubSub(t, nil, tc, cfg).runner,
		},
		{
			name:   "eventarc",
			runner: mustEventarc(t, nil, tc, cfg).runner,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.runner.eventsCompactionConfig != cfg {
				t.Errorf("eventsCompactionConfig = %v, want the config passed in", tt.runner.eventsCompactionConfig)
			}
		})
	}
}

func TestControllerCompactionDefaultsToNil(t *testing.T) {
	t.Parallel()
	...
	if c.runner.eventsCompactionConfig != nil {
		t.Errorf("eventsCompactionConfig = %v, want nil when no compaction is configured", c.runner.eventsCompactionConfig)
	}
}
...
func TestTriggerCompactionWarnsAboutSlidingWindows(t *testing.T) {
	tc := TriggerConfig{MaxConcurrentRuns: 1}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// The warning is emitted while the runner is built, before any
			// validation, so the controller itself does not matter here.
			_, _ = NewPubSubControllerWithConfig(ControllerConfig{TriggerConfig: tc, Compaction: tt.cfg})
...
func mustPubSub(t *testing.T, loader agent.Loader, tc TriggerConfig, cmp *compaction.Config) *PubSubController {
	t.Helper()
	c, err := NewPubSubControllerWithConfig(ControllerConfig{AgentLoader: loader, TriggerConfig: tc, Compaction: cmp})
	if err != nil {
...
func mustEventarc(t *testing.T, loader agent.Loader, tc TriggerConfig, cmp *compaction.Config) *EventarcController {
	t.Helper()
	c, err := NewEventarcControllerWithConfig(ControllerConfig{AgentLoader: loader, TriggerConfig: tc, Compaction: cmp})
	if err != nil {
		t.Fatalf("NewEventarcControllerWithConfig() error = %v", err)
	}
	return c
}
...
func TestControllerRefusesACompactionConfigItCannotServe(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := NewPubSubControllerWithConfig(ControllerConfig{
				SessionService: session.InMemoryService(),
				AgentLoader:    tt.loader,
				TriggerConfig:  tc,
				Compaction:     tt.cfg,
			})
			if gotOK := err == nil; gotOK != tt.wantOK {
				t.Errorf("NewPubSubControllerWithConfig() error = %v, want an error: %t", err, !tt.wantOK)
			}
		})
	}
}
...
func (stubSummarizer) SummarizeEvents(context.Context, []*session.Event) (compaction.SummarizeResult, error) {
